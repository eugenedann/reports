...
//$tper = mysql_fetch_array($tpeq , MYSQL_ASSOC);

if (!$filtype) {
	$filtype='reportType';
	$fildet=$fltr['reportType'];
}
$fillist=['Report','reportType','Status','Path','destDetail','destReport','Recurrence','Tag','Timestamp','DB','Table'];

if ($filtype != $fltr['Filter']) {
	$upds="update ReportFilter set Filter='$filtype'";
...
if ($filtype=='reportType') {
	if ($fildet != $fltr['reportType']) {
		$selrep='';
	}
	$sw=0;
	while ($tper = mysql_fetch_array($tpeq , MYSQL_ASSOC)) {
		//echo "$fildet == {$tper['ReportType']} - {$fltr['reportType']}<br>";
		if ($fildet==$tper['ReportType']) {			
			$sw=1;
		}		
	}
	if (!$sw) {
	...
	$upds="update ReportFilter set reportType='$fildet'";
	$updq = mysql_query($upds,$repc);
}  
if ($filtype=='Report') {
	if ($filtype != $fltr['Filter']) {
	...
	$upds="update ReportFilter set Report='$fildet'";
	$updq = mysql_query($upds,$repc);
} else if ($filtype=='destDetail') {
	if ($filtype != $fltr['Filter']) {
		$selrep='';
		$fildet='';
	}	
	$upds="update ReportFilter set destDetail='$fildet'";
	$updq = mysql_query($upds,$repc);
} else if ($filtype=='destReport') {
	if ($filtype != $fltr['Filter']) {
		$selrep='';
		$fildet='';
	}	
	$upds="update ReportFilter set destReport='$fildet'";
	$updq = mysql_query($upds,$repc);	
} 
if ($filtype=='Path') {
	if ($filtype != $fltr['Filter']) {
	...
	$upds="update ReportFilter set Path='$fildet'";
	$updq = mysql_query($upds,$repc);	
} 
if ($filtype=='Recurrence') {
	if ($filtype != $fltr['Filter']) {
		$selrep='';
		$fildet='';
	}	
	$upds="update ReportFilter set Recurrence='$fildet'";
	//echo "$upds <br>";
	$updq = mysql_query($upds,$repc);	
} 
if ($filtype=='DB') {
	...
	//if ($fildet) {
		$rds="select * from DatabaseDet where dbID='$fildet'";
		$rdq = mysql_query($rds,$repc);
		if ($rdr = mysql_fetch_array($rdq , MYSQL_ASSOC)) {
			$fildet=$rdr['dbID'];
		} else {
			$rds="select * from DatabaseDet order by DB limit 1";
			$rdq = mysql_query($rds,$repc);	
			$rdr = mysql_fetch_array($rdq , MYSQL_ASSOC);
			$fildet=$rdr['dbID'];
		}
	//}
	//echo "$rds <br>";
	$upds="update ReportFilter set DB='$fildet'";
	$updq = mysql_query($upds,$repc);	
} 
if ($filtype=='Tag') {
	if ($filtype != $fltr['Filter']) {
	...
	$upds="update ReportFilter set Tag='$fildet'";
	//echo "$upds <br>";
	$updq = mysql_query($upds,$repc);	
} 
if ($filtype=='Status') {
	...
	if ($fildet != "Active" && $fildet != "Paused" && $fildet != "Stopped" ) {
		$fildet=$fltr['Status'];
	}

	$upds="update ReportFilter set Status='$fildet', Filter='$filtype'";
	$updq = mysql_query($upds,$repc);
}

if ($filtype=='Timestamp') {
	if ($filtype != $fltr['Filter']) {
		$selrep='';
		$fildet='';
	}	

	$upds="update ReportFilter set Timestamp='$fildet', timestampType='$timestl'";
	$updq = mysql_query($upds,$repc);
}
$fltq = mysql_query($flts,$repc);
$fltr = mysql_fetch_array($fltq , MYSQL_ASSOC);
...
function freport($n) {
	...
	global $fltr,$filrep,$selcomp,$selrep, $frep, $repc, $ftype, $fpath, $frec, $fstatus, $ftme,$tst,$sts,$sdb,$stab,$stag;
	$wr="";
	...
	if ($fltr['Path']) {
		if ($wr) {
			//$wr .= " and Path like '%$fpath%' ";
			$wr .= " and Path like '%{$fltr['Path']}%' ";
		} else {
			//$wr = " where Path like '%$fpath%' ";
...
/*
	if ($srep) {
		$wfilt=1;
		if ($wr) {
			$wwr = $wr." and r.repID=$srep ";
		} else {
			$wwr = " where r.repID=$srep ";
		}

		$chkq = mysql_query($chks,$repc);
		if (!$chkr = mysql_fetch_array($chkq , MYSQL_ASSOC)) {
			$srep=0;
		}	
...
/////////////////////////////////////////////////// start FILTERS ////////////////////////////////////////
//echo "$filtype xxxx<br>";
//$fillist=['Report','reportType','Status','Path','destDetail','destReport','Recurrence','Tag','timestampType','Timestamp','DB','Table'];
$wa="<option>Clear</option>";

for($i=0;$i<count($fillist);$i++) {

	if ($fillist[$i]==$filtype) {
		$wa.= "<option selected>$fillist[$i]</option>";
	} else {
		$wa.= "<option>$fillist[$i]</option>";
	}
}
$wa.="</select>🔷";
if ($filtype =='reportType') {
	//$fts="select ReportType from Reports group by ReportType order by ReportType";
	//$ftq = mysql_query($fts,$repc);
	
	$wa.="<select name='fildet' id='fildet' onchange='this.form.submit()'>";
	sort($lstRepType);
	for ($i=0;$i<count($lstRepType);$i++) {
		if ($fildet==$lstRepType[$i]) {
			$wa.="<option selected>{$lstRepType[$i]}</option>";
		} else {
			$wa.="<option>{$lstRepType[$i]}</option>";
		}		
	}
/*
	while ($ftr = mysql_fetch_array($ftq , MYSQL_ASSOC)) {
		if ($fildet==$ftr['ReportType']) {
			$wa.="<option selected>{$ftr['ReportType']}</option>";
		} else {
			$wa.="<option>{$ftr['ReportType']}</option>";
		}
	}
*/
	$wa.="</select>🔷";
} else if ($filtype =='Status') {
	//$fts="select Status from Reports group by Status order by Status";
	//$ftq = mysql_query($fts,$repc);
	$wa.="<select name='fildet' id='fildet'  onchange='this.form.submit()'>";
	sort($lstStatus);
	for ($i=0;$i<count($lstStatus);$i++) {
		if ($fildet==$lstStatus[$i]) {
			$wa.="<option selected>{$lstStatus[$i]}</option>";
		} else {
			$wa.="<option>{$lstStatus[$i]}</option>";
		}		
	}
/*
	while ($ftr = mysql_fetch_array($ftq , MYSQL_ASSOC)) {
		if ($fildet==$ftr['Status']) {
			$wa.="<option selected>{$ftr['Status']}</option>";
		} else {
			$wa.="<option>{$ftr['Status']}</option>";
		}
	}
*/
	$wa.="</select>🔷";	
} else if ($filtype =='Timestamp') {
	$tstl=['DatabaseDet', 'Functions', 'Reports', 'Schedules', 'SQLstatements', 'TableDet'];
	$wa.="<select name='timestl' onchange='this.form.submit()'>";
	if (!$timestl) {
		$timestl='Reports';
	}
	...
	$wa.="</select>🔷";		
	$wa.="<input type='text' name='fildet' value='$fildet' onchange='this.form.submit()' />🔷"; 
} else if ($filtype =='DB') {

	$wa.="<select name='fildet' id='fildet'  onchange='this.form.submit()'>";
	for ($i=0;$i<count($lstDB);$i++) {
		$fts="select DB, Server from DatabaseDet where dbID='{$lstDB[$i]}'";
		$ftq = mysql_query($fts,$repc);
		$ftr = mysql_fetch_array($ftq , MYSQL_ASSOC);
		if ($fildet==$lstDB[$i]) {
			$wa.="<option value='{$lstDB[$i]}' selected>{$ftr['Server']} - {$ftr['DB']}</option>";
		} else {
			$wa.="<option value='{$lstDB[$i]}'>{$ftr['Server']} - {$ftr['DB']}</option>";
		}		
	}	
/*
	while ($ftr = mysql_fetch_array($ftq , MYSQL_ASSOC)) {
		...
		if ($fildet==$ftr['dbID']) {
			$wa.="<option value='{$ftr['dbID']}' selected>{$ftr['DB']}, {$ftr['Server']}</option>";
		} else {
			$wa.="<option value='{$ftr['dbID']}'>{$ftr['DB']}, {$ftr['Server']}</option>";
		}
	}
*/
	$wa.="</select>🔷";		
} else if ($filtype =='Table') {
	
} else if ($filtype =='Tag') {
	$fts="select Tag from Tags group by Tag order by Tag";
	$ftq = mysql_query($fts,$repc);
	$wa.="<select name='fildet' id='fildet'  onchange='this.form.submit()'>";

	while ($ftr = mysql_fetch_array($ftq , MYSQL_ASSOC)) {
		if ($fildet==$ftr['Tag']) {
			$wa.="<option selected>{$ftr['Tag']}</option>";
		} else {
			$wa.="<option>{$ftr['Tag']}</option>";
		}
	}
	$wa.="</select>🔷";	
} else if ($filtype =='Recurrence') {	
	$fts="select Recurrence from Schedules s
			inner join Reports r on r.repID=s.repID
			where ReportType = '{$fltr[reportType]}' group by Recurrence order by Recurrence";
	//$fts="select Recurrence from Schedules where ReportType='{$fltr[reportType]}' group by Recurrence order by Recurrence";
	//$ftq = mysql_query($fts,$repc);
	$wa.="<select name='fildet' id='fildet'  onchange='this.form.submit()'>";
	for ($i=0;$i<count($lstRecurrence);$i++) {
		...
		$ftq = mysql_query($fts,$repc);
		$ftr = mysql_fetch_array($ftq , MYSQL_ASSOC);
		if ($fildet==$lstRecurrence[$i]) {
			$wa.="<option selected>{$lstRecurrence[$i]}</option>";
		} else {
			$wa.="<option>{$lstRecurrence[$i]}</option>";
		}		
	}
/*
	while ($ftr = mysql_fetch_array($ftq , MYSQL_ASSOC)) {
		if ($fildet==$ftr['Recurrence']) {
			$wa.="<option selected>{$ftr['Recurrence']}</option>";
		} else {
			$wa.="<option>{$ftr['Recurrence']}</option>";
		}
	}
*/
	$wa.="</select>🔷";		
} else if ($filtype =='Report') {
	$wa.="<input type='text' name='fildet' id='fildet' value='{$fltr['Report']}' onchange='this.form.submit()' />🔷";
} else if ($filtype =='Path') {
	$wa.="<input type='text' name='fildet' id='fildet' value='{$fltr['Path']}' onchange='this.form.submit()' />🔷";
} else if ($filtype =='destDetail') {
	$wa.="<input type='text' name='fildet' id='fildet' value='{$fltr['destDetail']}' onchange='this.form.submit()' />🔷";
} else if ($filtype =='destReport') {
	$wa.="<input type='text' name='fildet' id='fildet' value='{$fltr['destReport']}' onchange='this.form.submit()' />🔷";
} else if ($filtype =='Timestamp') {
	$wa.="<input type='text' name='fildet' id='fildet' value='{$fltr['Timestamp']}' onchange='this.form.submit()' />🔷";
}

$ti = "Filter= {$fltr['Filter']} &#13";
$ti .= "reportType= {$fltr['reportType']} &#13";
$ti .= "Status= {$fltr['Status']} &#13";
$ti .= "Report= {$fltr['Report']} &#13";
$ti .= "destDetail= {$fltr['destDetail']} &#13";
$ti .= "destReport= {$fltr['destReport']} &#13";
$ti .= "Path= {$fltr['Path']} &#13";
$ti .= "Recurrence= {$fltr['Recurrence']} &#13";
$ti .= "Tag= {$fltr['Tag']} &#13";
$ti .= "timestampType= {$fltr['timestampType']} &#13";
$ti .= "Timestamp= {$fltr['Timestamp']} &#13";
$ti .= "DB= {$fltr['DB']} &#13";
$ti .= "Table= {$fltr['Table']} &#13";
$ti .= "******************************** &#13";
$ti .= "fildet= $fildet &#13";
		...
		if ($selcomp=='Tables') {
			echo "🔷<select name='seltab' id='seltab' onchange='this.form.submit()'>";
				...
				if ($seltab=="Link Table") {
					...
					echo "<option class='red' value='Link Table'>Link Table</option>";
				}	
